...
for thresh in thresholds:
    ...
    # Paths
    raw_path = f"CollecTF_Output_Reports/{thresh}_Threshold_OUTPUT_REPORT/{thresh}_raw_report_data.xlsx"
    best_path = f"CollecTF_Output_Reports/{thresh}_Threshold_OUTPUT_REPORT/{thresh}_best_conclusion_pipeline_data.xlsx"
    ...
    if os.path.exists(raw_path):
        ...
        agg_df = df_raw.groupby(['Species Protein', 'UniProt ID', 'Prospective Pattern']).agg({
            'Inverted_Pval': 'max',
            'correctness_%': 'mean',
...
plt.title("CollecTF System Performance: DR vs IR Optimization Metrics (p <= 0.05)")
...
ax3.set_title("CollecTF System Performance: Optimization Metrics (Optimal Significance Thresholds)")
...
ax4.set_title("CollecTF: Evolution of Optimal Significance Threshold Values")
...
summary_df.to_excel("CollecTF_Pipeline_Global_Threshold_Summary.xlsx", index=False)